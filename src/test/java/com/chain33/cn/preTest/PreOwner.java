package com.chain33.cn.preTest;
...
public class PreOwner {
    ...
    static RpcClient[]  preClient = new RpcClient[]{
            new RpcClient("http://121.37.90.80:11801"),
            new RpcClient("http://121.37.89.117:11801"),
            new RpcClient("http://121.37.87.25:11801"),
            new RpcClient("http://121.37.69.56:11801"),
    };

    // 区块链节点
    static RpcClient chain33Client = new RpcClient("http://121.37.69.56:8801");
    ...
    private void uploadKey(EncryptKey encryptKey, AccountInfo alice, String pubkey) throws IOException {
        ...
        for(int i = 0; i < preClient.length; i++) {
            boolean result = preClient[i].sendKeyFragment(alice.getPublicKey(), pubkey, encryptKey.getPubProofR(),
                    encryptKey.getPubProofU(), 1000, dhProof, kFrags[i]);
            if (!result) {
